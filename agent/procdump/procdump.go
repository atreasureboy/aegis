// Package procdump 提供进程内存转储。
// 使用 MiniDumpWriteDump (dbghelp.dll) 转储 Windows 进程。
//
// 面试要点：
// 1. 进程转储用途：提取 LSASS 内存中的明文密码、浏览器凭据等
// 2. Windows API：MiniDumpWriteDump (dbghelp.dll)
// 3. Dump 类型：MiniDumpWithFullMemory (0x00000002), MiniDumpWithDataSegs (0x00000001)
