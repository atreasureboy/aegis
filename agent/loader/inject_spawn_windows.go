...
func LoadWithSpawn(cfg *SpawnConfig) *LoadResult {
	...
	if err != nil {
		return &LoadResult{Success: false, Message: fmt.Sprintf("UTF16 encode exePath: %v", err)}
	}
	cmdLineUTF16, err := syscall.UTF16PtrFromString(cmdLine)
	if err != nil {
		return &LoadResult{Success: false, Message: fmt.Sprintf("UTF16 encode cmdLine: %v", err)}
	}
